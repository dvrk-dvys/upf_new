...
def generate_theory(board, verbose):
    ...
    for row, col in product(range(1, size + 1), repeat=2):
        clauses.append([var(row, col, n) for n in range(1, 10)])

        # for all values of row and column, a cell cannot take two diff (n) values
        for n in range(1, size + 1):
            for m in range(n + 1, size + 1):
                clauses.append([-var(row, col, n), -var(row, col, m)])

    # Consider which Value is chosen first in each slot
    for n in range(1, size + 1):
        pre = []
        for row, col in product(range(1, size + 1), repeat=2):
            pre.append([row, col, n])
            sets = []
            if len(pre) == size:
                for row, col, n in pre:
                    sets.append(var(row, col, n))
                if sets not in clauses:
                    clauses.append(sets)
                for row, col in combinations(sets, 2):
                    if [-row, -col] not in clauses:
                        clauses.append([-row, -col])
    ...
    for row, col in product(range(1, size + 1), repeat=2):
        ...
        if prep not in clauses:
            clauses.append(prep)
        prep = []

    # ---------A cell cannot take two diff (n) values-------------#
    # Clauses for row constraints
    ...
    # # Clauses for 3x3 constraints for box grid -> Every box contains every number:
...
def count_number_solutions(board, verbose=False):
    count = 1
    # TODO
    board_ = list(board.data.copy())
    clauses, init_variables, size, matrix = generate_theory(board, verbose)
    init_sat, init_soln = solve_sat_problem(clauses, "theory_c.cnf", size, init_variables, verbose, matrix)

    start = time.time()
    while True:
        board_iter = board_.copy()
        matrix_iter = [[0] * size for i in range(size)]

        for row, col in product(range(size), repeat=2):
            matrix_iter[row][col] = board_iter.pop(0)
        inverse = [(-1 * x) if (init_sat[x] == True) else x for x in init_sat]
        clauses.append(inverse[:-1])
        try:
            new_sat, new_soln = solve_sat_problem(clauses, "theory_final_all.cnf", size, init_variables, verbose, matrix_iter)
        except:
            transcurred = time.time() - start
            print(f'All solutions found below search time threshold, took {transcurred:.2f} seconds!')
            print('======================================================================')
            print(f'Number of solutions: {count}')
            return count

        # Adds the inverse of the already found solution as a new clause, excluding as well the already found
        if new_sat != init_sat and new_soln != init_soln:
            print(new_soln)
            count += 1
            inverse = [(-1 * x) if (new_sat[x] == True) else x for x in new_sat]
            clauses.append(inverse[:-1])

            # Check if already transcurred 1 minute, if so, return the number of solvers found
            transcurred = time.time() - start
            if transcurred > 60:
                print('Reached search time threshold!')
                print('======================================================================')
                print(f'Number of solutions: {count}')
                return count
        else:  # No more solutions found
            transcurred = time.time() - start
            print(f'All solutions found below search time threshold, took {transcurred:.2f} seconds!')
            print('======================================================================')
            print(f'Number of solutions: {count}')
...
def solve_sat_problem(clauses, filename, size, variables, verbose, matrix):
    ...
    return sat_assignment, solution
