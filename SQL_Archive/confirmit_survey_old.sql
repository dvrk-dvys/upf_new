...
--AND (TRUNC(convert_timezone('US/Pacific',cast((TIMESTAMP 'epoch' + CAST(w.icims_updated_timestamp AS BIGINT)/1000 * INTERVAL '1 Second') as timestamp))) >= '2020-01-01'
--       OR interview.event_finish_dt >= '2020-01-01')
--AND (TRUNC(convert_timezone('US/Pacific',cast((TIMESTAMP 'epoch' + CAST(w.icims_updated_timestamp AS BIGINT)/1000 * INTERVAL '1 Second') as timestamp))) > '2019-12-01'
--      OR interview.event_finish_dt > '2019-12-01')
--AND (TRUNC(convert_timezone('US/Pacific',cast((TIMESTAMP 'epoch' + CAST(w.icims_updated_timestamp AS BIGINT)/1000 * INTERVAL '1 Second') as timestamp))) >= dateadd(day,-8,sysdate)
--     OR interview.event_finish_dt >= dateadd(day,-8,sysdate))       
--AND DATEPART(YEAR, convert_timezone('US/Pacific',cast((TIMESTAMP 'epoch' + CAST(w.icims_updated_timestamp AS BIGINT)/1000 * INTERVAL '1 Second') as timestamp))) = 2019 
