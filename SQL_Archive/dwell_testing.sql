...
SELECT * 
FROM icimsprep ip
    INNER JOIN weeks wks ON ip.next_state_time BETWEEN wks.calendar_day AND (TRUNC(wks.calendar_day) + INTERVAL '6 DAY')
WHERE reporting_week_of_year = 31
and DATEDIFF(day, status_date, next_state_time) > 10 
AND first_state IN ( 'Candidate - Offer - Requested', 'Candidate - Offer - Approved', 'Candidate - Offer - Extended', 'Internal Transfer - Prepare Offer', 'Internal Transfer - Offer Confirmation Request', 'OFFER_CREATED', 'OFFER_EXTENDED')
