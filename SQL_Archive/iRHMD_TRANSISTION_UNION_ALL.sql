...
--NVL(team.team_flag, team2.team_flag, team3.team_flag) AS team_flag, 
    ...
    INNER JOIN masterhr.requisition reqs on reqs.job_icims_id = interview.job_icims_id AND reqs.current_transaction_flag = 'Y' AND clock.clock_stop_time BETWEEN reqs.snapshot_begin_timestamp and reqs.snapshot_end_timestamp
    ...
    FROM newish n
    --FROM oldone x
    left join oldone x ON n.email_address = x.email_address AND n.icims_id = x.icims_id and n.candidate_id = x.candidate_id
    WHERE 1=1
    --AND x.email_address IS NULL
    --OR n.email_address IS NULL
