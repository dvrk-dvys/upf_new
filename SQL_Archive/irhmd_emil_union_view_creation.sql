DROP VIEW opsdw.irhmd_email_union;
CREATE VIEW opsdw.irhmd_email_union AS ( 
...
WITH oldone AS (
    ...
    (
        ...
        where 1=1
        AND reqs.recruiter_reports_to_level_3_employee_login IN ('barresi','jsserran','jquintas','chaluleu','ninasj')
        OR reqs.recruiter_reports_to_level_4_employee_login IN ('barresi','jsserran','jquintas','chaluleu','ninasj')
        OR reqs.recruiter_reports_to_level_5_employee_login IN ('barresi','jsserran','jquintas','chaluleu','ninasj')
        OR reqs.recruiter_reports_to_level_6_employee_login IN ('barresi','jsserran','jquintas','chaluleu','ninasj')   
...
newish AS (
    ...
    INNER JOIN masterhr.requisition reqs on reqs.job_icims_id = interview.job_icims_id AND activity.application_date BETWEEN reqs.snapshot_begin_timestamp AND reqs.snapshot_end_timestamp --AND reqs.current_transaction_flag = 'Y' 
    INNER JOIN opsdw.ops_ta_team_wk team on team.emplid = reqs.recruiter_employee_id AND ctivity.application_date BETWEEN team.wk_begin_dt and team.wk_end_dt
 ...
 --   LEFT JOIN current_step cs on cs.job_icims_id = activity.job_icims_id and cs.candidate_icims_id = activity.candidate_icims_id  
  ...
  --  INNER JOIN clock_stop clock ON clock.candidate_icims_id = interview.candidate_icims_id AND clock.job_icims_id =interview.job_icims_id
    ...
    and clock.clock_stop_time::date >= dateadd(day,-7,sysdate)
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    SELECT DISTINCT 'New'::VARCHAR(200) AS metric_name,
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        SELECT DISTINCT 'Old'::VARCHAR(200) AS metric_name,
        full_name::VARCHAR(200),
        candidate_id::VARCHAR(200),
        empl_id::VARCHAR(200),
        application_date::VARCHAR(200),
...
SELECT DISTINCT      
u.full_name::VARCHAR(200),
u.candidate_id::VARCHAR(200),
u.empl_id::VARCHAR(200),
--u.application_date::VARCHAR(200),
n.application_date,
u.icims_id::VARCHAR(200),
--u.ofr_extndd_dt::VARCHAR(200),
n.ofr_extndd_dt,
--step::VARCHAR(200),
n.step,
--u.ofr_accepted_dt::VARCHAR(200),
n.ofr_accepted_dt,
u.loc_cty_nm::VARCHAR(200),
u.loc_bldg_cd::VARCHAR(200),
--u.business_title::VARCHAR(200),
n.business_title,
--u.external_job_title::VARCHAR(200),
n.external_job_title,
u.hrng_mngr_reports_to_level3_login::VARCHAR(200), 
u.co_nm::VARCHAR(200), 
u.req_dept_nm::VARCHAR(200), 
u.job_function_desc::VARCHAR(200),
u.job_type::VARCHAR(200), 
u.req_job_level::VARCHAR(200), 
u.rgltry_rgn_cd::VARCHAR(200),
u.email_address::VARCHAR(200),
u.job_title::VARCHAR(200), 
u.job_level::VARCHAR(200),
u.loc_nm::VARCHAR(200),
u.reports_to_level3_login::VARCHAR(200), 
u.reports_to_suprvsr_login::VARCHAR(200), 
u.dept_nm::VARCHAR(200), 
u.interview_date::VARCHAR(200), 
u.recruiter_login::VARCHAR(200),
u.recruiter_nm::VARCHAR(200),
u.Business_Lane::VARCHAR(200), 
--u.team_flag::VARCHAR(200), 
n.team_flag, 
u.intrvw_summary_id::VARCHAR(200),
u.rnum::VARCHAR(200)
FROM unionediRHMD_ETL u
LEFT JOIN newish n ON n.email_address = u.email_address AND n.icims_id = u.icims_id and n.candidate_id = u.candidate_id
WHERE 1=1
AND u.candidate_id || u.icims_id not in (select unique_id from opsdw.irhmd_email_union_historical)  
)
