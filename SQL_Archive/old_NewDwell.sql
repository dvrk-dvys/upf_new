...
hireraw AS (
      ...
      hmm.job_amzr_req_id
      ,hmm.cand_icims_id
      ,hmm.step AS current_status_hire
      ,hmm.job_art_job_id
      ...
      ,first_name, last_name, email, address_country, icims_id, req_status, sourcer_login, reports_to_level_6_employee_login, reports_to_level_6_employee_name, employee_login, relocations
),
...
icimsraw AS (
      ...
      fst.job_id
      ,fst.person_id
      ,fst.status AS current_status_icims
...
prep AS (
  ...
  FROM hireraw hr FULL OUTER JOIN icimsraw ir ON ir.person_id = hr.cand_icims_id AND ir.job_id = hr.job_amzr_req_id
),
...
allraw AS (
  ...
  SELECT DISTINCT *,
      ...
      (CASE 
       ...
       (CASE
          WHEN dwelling_now_hire = 'YES' AND dwelling_now_icims = 'YES' THEN 'YES'
          WHEN dwelling_now_hire IS NULL AND dwelling_now_icims = 'YES' THEN 'YES'
          WHEN dwelling_now_hire = 'YES' AND dwelling_now_icims IS NULL THEN 'YES'
          ELSE 'NO'
        END) AS dwelling_now
...
,originating_system
,dwelling_now
,COALESCE(CAST(job_amzr_req_id AS int), job_id) AS job_id
,COALESCE(CAST(cand_icims_id AS int), person_id) AS person_id
...
WHERE 1=1
--AND person_id
--AND job_id_final = 856550
--AND person_id_final = 34768
...
--AND reporting_week_of_year = 35
--AND dwelling_time = 33
--AND job_id = 856550
