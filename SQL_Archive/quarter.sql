...
( SELECT calendar_year, calendar_qtr FROM hrmetrics.o_reporting_days WHERE TRUNC(calendar_day) = TRUNC(sysdate))
,
...
INNER JOIN hrmetrics.o_reporting_days rd ON rd.calendar_day >= reqs.snapshot_begin_timestamp AND rd.calendar_day < snapshot_end_timestamp AND rd.calendar_qtr = 1 AND rd.calendar_year IN (2019) 
...
,rd.calendar_qtr as enter_state_qtr
...
,cq.calendar_qtr as current_qtr
...
INNER JOIN current_quarter cq ON cq.calendar_year >= r.calendar_year AND  cq.calendar_qtr >= r.calendar_qtr
